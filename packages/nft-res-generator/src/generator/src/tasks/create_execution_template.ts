...
import { dirname, fromFileUrl, join, resolve } from "https://deno.land/std@0.118.0/path/mod.ts";
...
const columns = ["id", "score", "raw_usd", "usd", "eth", "eth_actual", "is_listed", "is_purchased", "give_away_wallet"];
...
export function createExecutionTemplate() {
  ...
  const rarityTable: RarityTable = JSON.parse(readFile(`${metaDir}/rarity.json`));

  const scores: number[] = [];

  makeDir("build");
  writeFile(outFilePath, `${columns.join(",")}\n`);

  function calculateScoreFromAttributes(attributes: Attribute[], rarityTable: RarityTable): number {
  ...
  for (let id = 1; id <= 8888; id++) {
    const metadata = JSON.parse(readFile(`${metaDir}/${id.toString().padStart(4, "0")}.json`));
    // const metadata = JSON.parse(readFile(`${metaDir}/${id.toString()}.json`))
    const score = calculateScoreFromAttributes(metadata.attributes, rarityTable);
  ...
  function outputStats(scores: number[]): void {
    ...
    console.log(`Min: ${Math.min.apply(scores)}`);
    console.log(`Max: ${Math.max.apply(scores)}`);
